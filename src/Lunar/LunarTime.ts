...
export class LunarTime {
  ...
  constructor(
    ...
    this._ganIndex =
      ((this._lunar.getDayGanIndexExact() % 5) * 2 + this._zhiIndex) % 10;
  }
  ...
  getTianShen(): string {
    return LunarUtil.TIAN_SHEN[
      ((this._zhiIndex +
        LunarUtil.ZHI_TIAN_SHEN_OFFSET.get(this._lunar.getDayZhiExact())) %
        12) +
        1
    ];
  }
  ...
  getNineStar(): NineStar {
    ...
    let asc = false;
    if (
      solarYmd >= jieQi.get("冬至").toYmd() &&
      solarYmd < jieQi.get("夏至").toYmd()
    ) {
      asc = true;
    }
    let start = asc ? 7 : 3;
