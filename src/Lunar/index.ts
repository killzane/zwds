...
import { SolarUtil } from "../Solar/SolarUtil";
...
export class Lunar {
  ...
  static fromYmdHms(
    ...
    if (lunarDay > days) {
      throw new Error(
        `only ${days} days in lunar year ${lunarYear} month ${lunarMonth}`
      );
    }
  ...
  getMonthInChinese(): string {
    return (
      (this._month < 0 ? "闰" : "") + LunarUtil.MONTH[Math.abs(this._month)]
    );
  }
  ...
  private _getMonthPositionTaiSui(
    monthZhiIndex: number,
    monthGanIndex: number
  ): string {
    let p;
  ...
  getFestivals(): string[] {
    ...
    if (
      Math.abs(this._month) == 12 &&
      this._day >= 29 &&
      this._year != this.next(1).getYear()
    ) {
      l.push("除夕");
    }
    return l;
  ...
  getOtherFestivals(): string[] {
    ...
    if (solarYmd === jq.nextDay(-1).toYmd()) {
      l.push("寒食节");
    }
    ...
    if (solarYmd === jq.nextDay(offset + 40).toYmd()) {
      l.push("春社");
    }
  ...
  getDayNineStar(): NineStar {
    ...
    const dongZhiIndex = LunarUtil.getJiaZiIndex(
      dongZhi.getLunar().getDayInGanZhi()
    );
    const dongZhiIndex2 = LunarUtil.getJiaZiIndex(
      dongZhi2.getLunar().getDayInGanZhi()
    );
    const xiaZhiIndex = LunarUtil.getJiaZiIndex(
  ...
  getTimeNineStar(): NineStar {
    ...
    let asc = false;
    if (
      solarYmd >= this.getJieQiSolar("冬至").toYmd() &&
      solarYmd < this.getJieQiSolar("夏至").toYmd()
    ) {
      asc = true;
    ...
    let index = asc
      ? start + this._timeZhiIndex
      : start + 9 - this._timeZhiIndex;
  ...
  next(days: number): Lunar {
    return this._solar.nextDay(days).getLunar();
  }
  ...
  getFu(): Fu | null {
    const currentDay = Solar.fromYmd(
      this._solar.getYear(),
      this._solar.getMonth(),
      this._solar.getDay()
    );
    const xiaZhi = this._jieQi.get("夏至");
    const liQiu = this._jieQi.get("立秋");
    let startDay = Solar.fromYmd(
      xiaZhi.getYear(),
      xiaZhi.getMonth(),
      xiaZhi.getDay()
    );
  ...
  getWuHou(): string {
    ...
    const current = Solar.fromYmd(
      this._solar.getYear(),
      this._solar.getMonth(),
      this._solar.getDay()
    );
    const startSolar = jieQi.getSolar();
    const start = Solar.fromYmd(
      startSolar.getYear(),
      startSolar.getMonth(),
      startSolar.getDay()
    );
