...
export class EightChar {
  ...
  private static CHANG_SHENG_OFFSET: Dictionary<number> =
  ...
  getDayXun(): string {
    return 2 === this._sect
      ? this._lunar.getDayXunExact2()
      : this._lunar.getDayXunExact();
  }

  getDayXunKong(): string {
    return 2 === this._sect
      ? this._lunar.getDayXunKongExact2()
      : this._lunar.getDayXunKongExact();
  }
  ...
  getTimeDiShi(): string {
    ...
    let index =
      offset +
      (this._lunar.getDayGanIndexExact() % 2 == 0 ? zhiIndex : -zhiIndex);
    if (index >= 12) {
      index -= 12;
    }
    if (index < 0) {
  ...
  getMingGong() {
    ...
    let jiaZiIndex =
      LunarUtil.getJiaZiIndex(this._lunar.getMonthInGanZhiExact()) -
